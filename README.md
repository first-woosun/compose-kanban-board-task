...
# 2단계 - 칸반 보드 태스크(리팩터링)
## 수정사항 1. 작성자 컴포넌트 분리
- [ ] 기존 Icon과 writer 컴포넌트를 한 컴포저블에서 생성하던 코드를 Icon 생성 컴포저블과 writer 생성 컴포저블로 분해

## 수정사항 2. CardData 생성 방식 변경
- [ ] 기존 기본 생성자를 통해 객체를 생성하는 방식에서 companion object에 createCard 함수를 추가
- [ ] createCard 함수에 입력값 검증 추가
### 수정사항2 질문사항
1. init 키워드를 사용한 커스텀 생성자를 정의하여 입력 데이터를 검증하는 것과 현재 방식 중 어떤 것이 더 효율적일까요?

## 수정사항 3. 테스트 수행을 위한 컴포저블 testTag 추가
- [ ] 입력 데이터에 따른 컴포넌트 생성 여부 테스트를 위한 testTag 추가
...
- 컴포넌트의 가로 길이보다 내용이 긴 경우 글줄임표가 출력되는지 검증git 